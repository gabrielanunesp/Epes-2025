...
export const computeEA = (weights, team, refPrice, publicTarget) => {
    ...
    const eaLinear = weights.price * priceComponent +
...
export const softmax = (values, beta) => {
    if (values.length === 0)
        return [];
...
export const capShares = (shares, cap) => {
    if (!shares.length)
        return shares;
    if (cap === undefined || cap === null)
        return shares;
    const safeCap = Math.max(0, Math.min(1, cap));
    const result = new Array(shares.length).fill(0);
    const fixed = new Array(shares.length).fill(false);
...
export const demandRaw = (marketSize, share, price, refPrice, elasticity) => {
...
export const unitCost = (team, season) => {
...
export const windfallDamping = (profits, alpha) => {
    const safeAlpha = alpha ?? 0;
    if (safeAlpha <= 0 || profits.length === 0)
        return profits.slice();
...
export const splitProfit = (profit, reinvestRate) => {
    if (profit <= 0) {
