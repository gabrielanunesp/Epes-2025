...
const ensurePositive = (value, fallback = 0) => (Number.isFinite(value) && value > 0 ? value : fallback);
...
const mapPublicTarget = (publicTargets, id) => {
    const map = indexById(publicTargets);
    return map.get(id);
};
const computeBenefitCost = (team, season) => {
...
export const computeRound = (input) => {
    ...
    const results = enriched.map((item, idx) => {
    ...
    const totals = finalResults.reduce((acc, result) => {
